...
for N in Ns:
    ...
    for rep in range(reps):
        ...
        x = torch.tensor([[1,0]*int(N/2)]) if N%2==0 else torch.tensor([[1,0]*int(N/2)+[1]])  # The 1010101... single basis state
        ...
        basis_m_n_half = torch.tensor(binary_basis(geometry=(N//2,)))
        basis_m_n = torch.tensor(binary_basis(geometry=(N,)))

        probs = torch.zeros(2**(N//2), dtype=torch.cfloat)
        ...
        n_batches = len(basis_m_n_half)//10 if len(basis_m_n_half)%10 == 0 else len(basis_m_n_half)//10+1
        # n_batches = len(basis_m_n)//10 if len(basis_m_n)%10 == 0 else len(basis_m_n)//10+1
        
        for i in range(n_batches):
            y_batch = basis_m_n_half[10*i : 10*(i+1)]
            # y_batch = basis_m_n[10*i : 10*(i+1)]

            x_batch = x.repeat(y_batch.shape[0], 1) # a batch of 10
            sub_mask_batch = (torch.tensor([ [1]*(N//2)+[0]*(N//2) ])).repeat(y_batch.shape[0], 1) # Measure the first half
            # sub_mask_batch = (torch.tensor([ [1]*N ])).repeat(y_batch.shape[0], 1) 
            print('y_batch, x_batch, sub_mask_batch', (y_batch, x_batch, sub_mask_batch))
        ...
        rho_A = qiskit.quantum_info.partial_trace(state_exact, [i for i in np.arange(N//2)])
        # rho_A = qiskit.quantum_info.partial_trace(state_exact, [i for i in np.arange(N//2, N)])

        rho_test = qiskit.quantum_info.partial_trace(state_exact, [i for i in np.arange(N-1)])
        print('rho_test', rho_test)
        # rho_A = qiskit.quantum_info.partial_trace(state_exact, [])

        probs_exact = (np.abs(np.diag(rho_A.data))).squeeze()
        ...
        #print('debug amps', amps)
        #q_fidelity1 = qiskit.quantum_info.state_fidelity(qiskit.quantum_info.Statevector(amps), rho_A)
